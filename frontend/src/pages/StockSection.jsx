...
const StockAnalysis = ({ setError }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/analyze_historical', formData, {
      ...
      <div className="flex flex-col items-center py-6">
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={chartData} margin={{ top: 20, right: 20, left: 0, bottom: 40 }}>
            ...
            <Tooltip
              formatter={value => [`$${value.toLocaleString()}`, 'Predicted Close']}
      ...
      <Card className="w-full max-w-2xl mt-6 bg-blue-100  shadow-xl">
        ...
        {analysisData && renderChart()}
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-2">Upload Historical Data</label>
              <Input type="file" name="historical_data" onChange={handleFileChange} className="w-full bg-gray-100" />
            </div>
