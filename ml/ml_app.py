...
if not mongo_uri:
    try:
        ...
        mongo_uri = 'mongodb+srv://root:small_business@smallbusinessplanner.y9gdu.mongodb.net/?retryWrites=true&w=majority&appName=smallBusinessPlanner'
    except ImportError:
...
db = client["smallBusinessPlanner"]
predictions_collection = db["StockPredictions"] 
...
def train_lstm_model(X_train, y_train):
    try:
        ...
        model.add(LSTM(units=30, return_sequences=True, input_shape=(X_train.shape[1], 1), activation='tanh'))
        ...
        # Train model
        model.fit(X_train, y_train, epochs=60, batch_size=120)
